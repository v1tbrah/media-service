...
func TestAPI_GetPost(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			req:  &mpbapi.GetPostRequest{Guid: "12345678"},
			mockStorage: func(t *testing.T) *mocks.Storage {
				testStorage := mocks.NewStorage(t)
				testStorage.On("GetPost",
					mock.MatchedBy(func(ctx context.Context) bool { return true }), "12345678").
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr {
				...
				assert.Equal(t, tt.expectedData, resp.GetData())
			}
		})
	}
}
